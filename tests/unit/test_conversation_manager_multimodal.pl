...
subtest 'enforce_message_alternation does NOT merge consecutive system messages' => sub {
    my $messages = [
        ...
        make_msg('system', 'Dynamic UC: active todos and compressed tail summary'),
    ...
    is($result->[5]{content}, 'Dynamic UC: active todos and compressed tail summary', 'dynamic UC section preserved');
    ...
    is($result->[5]{role}, 'system', 'position 5 is system (dynamic UC at fixed position)');
