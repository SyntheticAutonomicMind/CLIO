...
#   string (e.g. "openrouter/poolside/laguna-s-2.1") and
#   _check_provider_auth was called with that bogus "provider" name,
#   producing the wrong error:
#     Provider 'openrouter/poolside/laguna-s-2.1' has no API key configured.
#     Set it with: /api set provider openrouter/poolside/laguna-s-2.1 ...
#
...
subtest '_resolve_model_details returns 4-tuple (full, display, target_provider, api_model)' => sub {
    ...
    my $value = 'openrouter/poolside/laguna-s-2.1';
    ...
    is($api_model, 'poolside/laguna-s-2.1', 'api_model is the part after the first slash');
    is($full_model, 'openrouter/poolside/laguna-s-2.1', 'full_model is the input unchanged');
};

subtest 'multi-slash model IDs split on first slash only' => sub {
    require CLIO::Providers;

    my $value = 'kilo/deepseek/deepseek-v4-flash';
    my $current_provider = 'openrouter';
    ...
    is($api_model, 'deepseek/deepseek-v4-flash', 'multi-slash: api_model keeps the rest');
    is($full_model, 'kilo/deepseek/deepseek-v4-flash', 'multi-slash: full_model unchanged');
};

subtest 'unprefixed model uses current provider' => sub {
    require CLIO::Providers;

    my $value = 'laguna-s-2.1';
    my $current_provider = 'openrouter';
    ...
    is($full_model, 'openrouter/laguna-s-2.1', 'unprefixed: full_model gets current prefix');
    is($api_model, 'laguna-s-2.1', 'unprefixed: api_model is the input as-is');
};
...
subtest 'Config model_routes round-trip' => sub {
    ...
    ok($config->set_model_route('laguna', \@models), 'set_model_route returns true');

    my $route = $config->get_model_route('laguna');
    ok($route, 'route is retrievable');
    is(scalar(@$route), 3, 'route has 3 models');
    is($route->[0], 'openrouter/poolside/laguna-s-2.1', 'first model preserved verbatim');
    ...
    is($routes{'laguna'}, $route, 'list hash maps name to arrayref');

    # delete
    ok($config->delete_model_route('laguna'), 'delete_model_route returns true');
    is($config->get_model_route('laguna'), undef, 'route is gone after delete');
    ...
    ok($config2->set_model_route('saved-route', ['openrouter/x']), 'set after reload');
    $config2->save();
    my $config3 = CLIO::Core::Config->new(config_dir => $dir);
    is_deeply($config3->get_model_route('saved-route'), ['openrouter/x'],
...
subtest '2-value unpack reproduces the bug; 4-value unpack fixes it' => sub {
    ...
    is($provider_buggy, 'openrouter/poolside/laguna-s-2.1',
    ...
    is($api, 'poolside/laguna-s-2.1', 'FIXED: $api_model is the model part');
};
