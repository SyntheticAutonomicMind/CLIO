...
subtest 'config model candidates' => sub {
    ...
    is($candidates->[0], 'openrouter/foo', 'first candidate correct');
    is($candidates->[2], 'vercel/baz', 'third candidate correct');
    ...
    is($candidates2->[0], 'openrouter/foo', 'first candidate survives reload');
...
subtest 'api_manager model routing' => sub {
    ...
    # Cycle to next model
    is($api->get_current_model(), 'openrouter/foo', 'initial model correct');

    my ($new, $old) = $api->cycle_model();
    is($new, 'kilo/bar', 'cycled to second model');
    is($old, 'openrouter/foo', 'old model was first');
    is($api->get_current_model(), 'kilo/bar', 'current model updated');
    is($config->get_model_routing_index(), 1, 'routing index is 1');

    # Cycle again
    ($new, $old) = $api->cycle_model();
    is($new, 'vercel/baz', 'cycled to third model');
    is($config->get_model_routing_index(), 2, 'routing index is 2');

    # Cycle again - should wrap to first
    ($new, $old) = $api->cycle_model();
    is($new, 'openrouter/foo', 'wrapped to first model');
...
subtest 'error handler model routing' => sub {
    ...
    my $api = CLIO::Core::APIManager->new(debug => 0, config => $config);
    my @models = ('openrouter/foo', 'kilo/bar');
    $config->set("model_candidates", \@models, 0);
    $config->set("model_routing_index", 0, 0);
    $config->set("model", 'openrouter/foo', 0);
    ...
    is($api->get_current_model(), 'kilo/bar', 'model switched to kilo');
    is($session->{routing_attempts}, 1, 'routing attempts incremented');
    is($sys_msgs[0], 'API Rate Limit (429), rerouting to kilo/bar', 'system message includes error type and status code');
...
subtest 'error handler routing exhaustion' => sub {
    ...
    for my $attempt (1..20) {
        ...
        if (ref($result) eq 'HASH') {
            ...
            ok($result->{error} =~ /last: vercel\/baz/, "attempt $attempt: error includes last model name");
            $gave_up = 1;
            last;
        }
...
subtest 'route_verbose=0 suppresses rerouting system message' => sub {
    ...
    is($api->get_current_model(), 'kilo/bar', 'but routing still happened');
    `rm -rf $dir`;
};
...
subtest 'route_verbose=1 shows rerouting system message' => sub {
    ...
    like($sys_msgs[0], qr/rerouting to kilo\/bar/, 'message names the new model');
    `rm -rf $dir`;
};
...
subtest 'non-actionable errors skip routing entirely' => sub {
    ...
    # model_not_found - should not cycle, falls through to non-retryable path
    {
        ...
        my $result = CLIO::Core::API::ErrorHandler::handle_api_error($wo, $api_response, $ctx);
        is($api->get_current_model(), 'openrouter/foo', 'model_not_found: did not cycle models');
    ...
    $session = { routing_attempts => 0 };
    {
        ...
        my $result = CLIO::Core::API::ErrorHandler::handle_api_error($wo, $api_response, $ctx);
        is($api->get_current_model(), 'openrouter/foo', 'billing_error: did not cycle models');
    }
    ...
    $session = { routing_attempts => 0 };
    {
        ...
        my $result = CLIO::Core::API::ErrorHandler::handle_api_error($wo, $api_response, $ctx);
        is($api->get_current_model(), 'openrouter/foo', 'auth_failed: did not cycle models');
    }
    ...
    $session = { routing_attempts => 0 };
    {
        ...
        my $result = CLIO::Core::API::ErrorHandler::handle_api_error($wo, $api_response, $ctx);
        is($api->get_current_model(), 'openrouter/foo', 'weekly rate limit: did not cycle models');
    }
...
subtest 'all-providers rate-limited jumps to soonest-expiring (excludes current)' => sub {
    ...
    is($api->get_current_model(), 'kilo/bar', 'current model is kilo (soonest)');
...
subtest 'no routing with single model' => sub {
    ...
    $config->set("model_candidates", ['openrouter/foo'], 0);
    $config->set("model_routing_index", 0, 0);
...
subtest 'routing message includes error type for server error' => sub {
    ...
    is($sys_msgs[0], 'API Server Error (500), rerouting to kilo/bar', 'server error message includes type and status');
...
subtest 'provider_for_model resolves provider from model prefix' => sub {
    ...
    is($api->provider_for_model('openrouter/foo'), 'openrouter', 'openrouter provider');
    is($api->provider_for_model('kilo/bar'), 'kilo', 'kilo provider');
    is($api->provider_for_model('vercel/baz'), 'vercel', 'vercel provider');
...
subtest 'rate_limit during routing records per-provider cooldown' => sub {
    ...
    is($api->get_current_model(), 'kilo/bar', 'cycled to kilo');
...
subtest 'route_wait_for_rate_limits=0 disables rate-limit waiting' => sub {
    ...
    is($api->get_current_model(), 'kilo/bar', 'still cycles to next model');
