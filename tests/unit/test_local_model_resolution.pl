...
$cfg3->set('model', 'openrouter/meta-llama/llama-3.1-405b-instruct');
$cfg3->set('api_key', 'dummy');

my $mgr3 = CLIO::Core::APIManager->new(config => $cfg3);
my $ep3 = $mgr3->_prepare_endpoint_config();

if ($ep3 && $ep3->{model} && $ep3->{model} eq 'meta-llama/llama-3.1-405b-instruct') {
    pass("OpenRouter model preserved: $ep3->{model}");
} else {
    fail("Expected 'meta-llama/llama-3.1-405b-instruct', got '$ep3->{model}'");
}
