...
subtest 'set_model_route overwrites (does not append)' => sub {
    # Seed a route.
    ok($cfg->set_model_route('laguna', ['openrouter/a', 'kilo/b']), 'seed route');
    is_deeply($cfg->get_model_route('laguna'), ['openrouter/a', 'kilo/b'],
        'seed route has 2 models');

    # Replace - should overwrite, not append.
    ok($cfg->set_model_route('laguna', ['openrouter/x', 'kilo/y', 'vercel/z']),
        'replace returns true');
    my $after = $cfg->get_model_route('laguna');
    is(scalar(@$after), 3, 'route now has 3 models (overwrote, did not append)');
    is($after->[0], 'openrouter/x', 'first model replaced');
    is($after->[2], 'vercel/z', 'third model replaced');
};

subtest 'case-insensitive route lookup' => sub {
    # set_model_route lowercases the name on store.
    is_deeply($cfg->get_model_route('LAGUNA'), ['openrouter/x', 'kilo/y', 'vercel/z'],
...
subtest 'config persistence keeps replaced route' => sub {
    ...
    is($cfg2->get_model_route('laguna')->[0], 'openrouter/x', 'first model survives reload');
    remove_tree($dir);
};
...
subtest 'handle_route dispatches replace -> _route_replace' => sub {
    ...
    $cmd->handle_route('replace', 'myname', 'openrouter/foo', 'kilo/bar');
    is($cmd->{_dispatched}, 'replace', 'handle_route dispatched replace');
    is_deeply($cmd->{_route_replace_called_with}, ['myname', 'openrouter/foo', 'kilo/bar'],
