...
#
# Bug: After /api route use minimax (whose first model is
# "openrouter/minimax/minimax-m3"), the global config still had
...
subtest 'reported bug: /api route use minimax leaves provider=github_copilot' => sub {
    ...
    # Save the route "minimax" with the user's model list
    my @models = (
        'openrouter/minimax/minimax-m3',
        'vercel/minimax/minimax-m3',
    );
    ok($config->set_model_route('minimax', \@models), 'route saved');
    ...
    is($config->get('model'), 'openrouter/minimax/minimax-m3',
        'POST: model is the route first model');
    is($config->get('api_key'), 'sk-or-v1-test',
        'POST: api_key is the openrouter stored key');
    is($config->get('route_name'), 'minimax',
        'POST: route_name set');
    is_deeply($config->get_model_route('minimax'), \@models,
...
subtest 'reported bug: /api route use when route first model is already current provider' => sub {
    ...
    is($config->get('model'), 'openrouter/foo', 'model updated to route first');
...
subtest '/api set model openrouter/foo also switches provider' => sub {
    ...
    my $cmd = make_cmd($config);
    $cmd->_set_model('openrouter/foo', 0);
...
subtest 'session_only: provider override stored in session state, not global' => sub {
    ...
    $cmd->_set_model('openrouter/foo', 1);  # 1 = session_only
    ...
    is($state->{api_config}{model}, 'openrouter/foo',
