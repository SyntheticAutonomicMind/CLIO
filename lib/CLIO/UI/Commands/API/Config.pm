...
sub _route_add {
    ...
    unless (@models >= 1) {
        ...
        $self->writeline("Example: /api route add laguna openrouter/foo kilo/bar", markdown => 0);
        return;
    }
...
Example:
  /api route replace laguna openrouter/foo kilo/bar
...
sub _route_replace {
    ...
    unless (@models >= 1) {
        $self->display_error_message("Must specify at least one model");
        $self->writeline("Example: /api route replace $name openrouter/foo kilo/bar", markdown => 0);
        return;
    }
...
Arguments:
  $full_model      - Fully-prefixed model name (e.g. "openrouter/foo")
