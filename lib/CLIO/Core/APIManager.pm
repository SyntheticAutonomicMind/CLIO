...
Arguments:
  $model - Model string (e.g. "openrouter/foo", "kilo/bar")
...
sub adapt_request_for_endpoint {
    ...
    if (exists $payload->{tools} && $payload->{model}) {
        ...
        # 'minimax/minimax-m2.7' would be mis-parsed as the CLIO
    ...
    # Data-driven reasoning parameter injection via reasoning_schema.
    # Handles all provider-specific thinking formats (OpenAI/DeepSeek/NVIDIA
    # effort, OpenRouter nested, MiniMax think_object, Z.AI mixed) without
    # per-provider if-blocks. The reasoning_schema is propagated from
    # provider-defaults.json by build_endpoint_config(). See
    # _inject_reasoning_params() for the full dispatch.
...
sub _inject_reasoning_params {
    ...
    # bug: stripped 'minimax/minimax-m2.7' would be mis-parsed as the
...
# "minimax/minimax-m2.7" gets mis-parsed as the CLIO "minimax" provider
