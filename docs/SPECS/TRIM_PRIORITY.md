...
**Lifecycle:** Slot size locked to `MIN_CSSS_SLOT_TOKENS` (8K) at
first trim, may grow up to `MAX_CSSS_SLOT_TOKENS` (12K) if dropped
content is >1.5x the current slot. The summary content rotates but the
slot size is stable - that's the CSSS guarantee.
...
| Name | Value | Purpose |
|------|-------|---------|
| `MIN_CSSS_SLOT_TOKENS` | 8192 | First-trim CSSS slot size floor. |
| `MAX_CSSS_SLOT_TOKENS` | 12000 | CSSS slot size ceiling. |
