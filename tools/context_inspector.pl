...
sub _find_tool_errors {
    ...
    # Show the last 30 errors (most recent first). Use a safe slice
    # so we don't go past the end of the array.
    my $last_n = 30;
    my @to_show = @err_idx >= $last_n ? @err_idx[-$last_n .. -1] : @err_idx;
    for my $i (reverse @to_show) {
        my $m = $hist->[$i];
        my $c = $m->{content} // '';
