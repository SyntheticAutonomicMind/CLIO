...
if ($json_output) {
    # ModelBudget.pm was removed from the codebase; if not present,
    # skip the model class field rather than dying.
    my $detected_class;
    if (eval { require CLIO::Core::ModelBudget; 1 }) {
        my $ctx = $j->{max_tokens} || 0;
        $detected_class = $model_class
...
# (ModelBudget.pm was removed from the codebase; skip if not present.)
